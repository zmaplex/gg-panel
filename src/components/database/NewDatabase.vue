  ...
  <q-card class="card q-pa-md">
    <div v-if="ui.step==='create'">
      ...
      <q-card-actions align="right">
        <q-btn color="blue-grey" label="Create" @click="Public.requestCreateNewDatabase()"></q-btn>
      </q-card-actions>
    </div>
    ...
    <div v-else-if="ui.step === 'failed'">
      ...
      <q-card-section>
        ...
        <q-input v-model="ui.errorMsg" readonly type="textarea">
        </q-input>
      </q-card-section>
  ...
  setup() {
    ...
    const Public = {
      requestCreateNewDatabase() {
        createNewDatabase(data.value.newDatabaseForm).then(res => {
          console.log(res)
