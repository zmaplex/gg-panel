<template>
  <q-card class="shadow-0">
    <q-card-section class="flex q-gutter-sm">
      <div v-if="ui.lasted.iBytes"> IN: {{
          format.humanStorageSize(ui.lasted.iBytes)
        }}
      </div>

      <div v-if="ui.option.series[0].data.slice(-1)">
        {{format.humanStorageSize(ui.option.series[0].data.slice(-1)[0][1])}} /sec
      </div>
      <div v-if="ui.lasted.oBytes">OUT: {{ format.humanStorageSize(ui.lasted.oBytes)}}</div>
      <div v-if="ui.option.series[1].data.slice(-1)">
        {{format.humanStorageSize(ui.option.series[1].data.slice(-1)[0][1])}} /sec
      </div>
    </q-card-section>
    <q-card-section>
...
export default {
  ...
  setup() {
    ...
    websocket.onmessage = function (e) {
      ...
      if (data.code === 201) {
        let sql = "select *, 'interface_details' as _qtype  from  interface_details where interface != 'lo';"
        let query = {
          'query_sql': sql,
        ...
        if (lasted.oBytes == null) {
          ...
          let i_speed = (Number(net_interface.ibytes) - lasted.iBytes) / interval.value
          let o_speed = (Number(net_interface.obytes) - lasted.oBytes) / interval.value
